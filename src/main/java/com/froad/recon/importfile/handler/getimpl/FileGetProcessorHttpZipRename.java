...
/**
 * 获取对账文件(ZIP压缩文件解压指定文件名)
 * @author Administrator
 *
 */
public class FileGetProcessorHttpZipRename implements FileGetProcessor {

	final static Logger logger = LoggerFactory.getLogger(FileGetProcessorHttpZipRename.class);
	...
	public Map<String,Object> execute(Map<String,Object> reqMap) throws FlowException{
		...
		try {
			...
			String localFilePath = String.format(platformDetail.getLocalFilePath(), transDate);
			
			//解压到指定文件名
			FileUtil.unZipFile(filePath, fileDir, localFilePath);
			logger.info("解压对账文件成功.[filePath={},fileDir={},localFilePath={}]", 
					new Object[]{filePath, fileDir, localFilePath});
