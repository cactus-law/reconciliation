 ...
 *
 */
public class FileGetProcessorHttpAnhuiZip implements FileGetProcessor {

	final static Logger logger = LoggerFactory.getLogger(FileGetProcessorHttpAnhuiZip.class);
	...
	public Map<String,Object> execute(Map<String,Object> reqMap) throws FlowException{
		...
		try {
			...
			for (String channe : FileConstants.CHANNE_LIST) {
				fileUrl = String.format(fileUrlTemp, temp,channe);
				
				filePath = ParamCommand.data_dir+transDate+"/"+platformDetailNo+"/"+channe+".zip";
