 ...
 * 日切openapi.pay流程
 */
@Component("flowRqOpenApiPay")
public class FlowRqOpenApiPay implements FlowInterface {
    private final static Logger logger = Logger.getLogger(FlowRqOpenApiPay.class);
    private static boolean locked = false;
    @Autowired
    private RqService rqService;

    public Rckflowdetail execute(Rckflowdetail rckflowdetail) throws FlowException {
        logger.info("日切openapi.pay流程开始!");
        if (!locked) {
            locked = true;
            ...
            try {
                ...
                int insertNum = rqService.rqOpenApiPay(startDate, endDate);
            ...
            logger.error("openapi.pay交易日切已在运行");
            throw new FlowException(rckflowdetail, "openapi.pay交易日切正在运行......");
        }
