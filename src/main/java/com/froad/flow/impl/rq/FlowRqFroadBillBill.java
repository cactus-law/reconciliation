 ...
 * 日切froadbill.bill流程
 */
@Component("flowRqFroadBillBill")
public class FlowRqFroadBillBill implements FlowInterface {
    private final static Logger logger = Logger.getLogger(FlowRqFroadBillBill.class);
    private static boolean locked = false;
    @Autowired
    private RqService rqService;

    public Rckflowdetail execute(Rckflowdetail rckflowdetail) throws FlowException {
        logger.info("日切froadbill.bill流程开始!");
        if (!locked) {
            locked = true;
            ...
            try {
                ...
                int insertNum = rqService.rqFroadBillBill(startDate, endDate);
            ...
            logger.error("froadbill.bill交易日切已在运行");
            throw new FlowException(rckflowdetail, "froadbill.bill交易日切正在运行......");
        }
